...
function parseXml(data) {
    ...
    const results = nodes.map(n => {
        ...
        if (req.length) {
            result.id = aggPharagraphs(req[1], false, this.settings.convertHtmlToJiraMarkup);
            result.type = aggPharagraphs(req[2], false, this.settings.convertHtmlToJiraMarkup);
            result.description = aggPharagraphs(req[3], true, this.settings.convertHtmlToJiraMarkup);
...
function updateXml(data, map) {
    ...
    nodes.forEach(n => {
        ...
        if (req.length) {
            const id = aggPharagraphs(req[1], false, this.settings.convertHtmlToJiraMarkup);

            console.log(`Mapping ${id} to ${map[id]}`);

            const macro = `<ac:structured-macro ac:name="jira" ac:schema-version="1" ac:macro-id="39b30d60-4df7-44bc-a639-4427802f1cb0"><ac:parameter ac:name="server">JIRA (${this.settings.host})</ac:parameter><ac:parameter ac:name="columns">key,summary,type,created,updated,due,assignee,reporter,priority,status,resolution</ac:parameter><ac:parameter ac:name="serverId">b544e6f4-ac54-35f8-a63d-6d1ebc715244</ac:parameter><ac:parameter ac:name="key">${map[id]}</ac:parameter></ac:structured-macro>`;
    ...
    let afterCleanup = xmlString.replace(/ ?xmlns:.+?=""/g, '');

    // first remove all extra &amp; that serializator added
    afterCleanup = afterCleanup.replace(/&amp;(?!&)(.{1,6}?;)/g, '&$1');
    console.log('After --->', afterCleanup, '<----------');
    return afterCleanup;
